 ...
 */
vector<BIGNUM*> readBIGNUMsFromFile(char* filename) {

	cout << ("in readBIGNUMsFromFile") << endl;
	...
	if (!infile.is_open()) {
		...
		return data_list; // 返回空的 vector
	}

	cout << ("find fileName and to read") << endl;
	while (getline(infile, line)) {
		BIGNUM* bn = BN_new();
		if (bn == NULL) {
			cout << ("create bn error") << endl;
		}
...
EXPORT_SYMBOL int deal(char* algoName, char* fileString, char* resultFilePath) {
	...
	else if (strcmp(algoName, "compare") == 0) {
		vector<BIGNUM*> data_list = readBIGNUMsFromFile(fileString);
	...
	else if (strcmp(algoName, "equal") == 0) {
		vector<BIGNUM*> data_list = readBIGNUMsFromFile(fileString);
	...
	else if (strcmp(algoName, "min_max") == 0) {
		vector<BIGNUM*> data_list = readBIGNUMsFromFile(fileString);
	...
	else if (strcmp(algoName, "include") == 0) {
		vector<BIGNUM*> data_list = readBIGNUMsFromFile(fileString);
	...
	else if (strcmp(algoName, "intersect") == 0) {
		vector<BIGNUM*> data_list = readBIGNUMsFromFile(fileString);
	...
	else if (strcmp(algoName, "inner_product") == 0) {
	...
	else if (strcmp(algoName, "distance") == 0) {
		vector<vector<BIGNUM*>> data_list(2);
		data_list[0] = readBIGNUMsFromFile(fileString, 1);
		data_list[1] = readBIGNUMsFromFile(fileString, 2);
	...
	else if (strcmp(algoName, "split") == 0) {
		vector<vector<BIGNUM*>> data_list(2);
		data_list[0] = readBIGNUMsFromFile(fileString, 1);
		data_list[1] = readBIGNUMsFromFile(fileString, 2);
	...
	else if (strcmp(algoName, "frequency") == 0) {
		vector<vector<BIGNUM*>> data_list(2);
		data_list[0] = readBIGNUMsFromFile(fileString, 1);
		data_list[1] = readBIGNUMsFromFile(fileString, 2);
		// 将data_list[0][0]转化为int类型
